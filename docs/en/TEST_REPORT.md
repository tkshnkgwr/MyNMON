...
| **Network**         | Press `n` key       | Toggles Rx/Tx network interface metrics.                    | **PASS** | Active interfaces are sorted alphabetically; no shifts. |
| **Process**         | Press `p` or `t`    | Toggles the active process list.                            | **PASS** | Top 8 CPU-bound processes are sorted and formatted.     |
| **Filter**          | Press `f`, type text| Filters processes in real-time; displays matching counts.   | **PASS** | `Enter` or `Esc` exits filter mode; filter persists.    |
| **Diff Log**        | Press `g` or `l`    | Toggles process spawn/exit diff log.                        | **PASS** | Renders green `+` for spawns and red `-` for exits.     |
| **Double-Launch**   | Run two instances   | Second instance terminates with an error code of `1`.       | **PASS** | Outputs: `Error: Another instance of MyNMON is running.`|
| **Exit**            | Press `q` or `Esc`  | Process exits immediately; terminal state is restored.      | **PASS** | Restores cursor, raw mode disabled, alternate screen off|
...
| **Flicker-Free**    | Change sections     | Layout redrawing does not trigger terminal flickering.      | **PASS** | Upwards cursor resets and line-end clears function well.|
...
| **Change Interval** | Press `r`, type `3` | Refresh interval dynamically updates to 3 seconds.          | **PASS** | Tick rate modifies metrics; header shows `Interval: 3s`.|
